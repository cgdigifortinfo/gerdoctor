...
## Completed (recent)
- [x] 2026-04-21: **Admin User-Liste zeigt Partner-Namen** — neue "Partner"-Spalte zwischen Role und Progress. Backend `/api/admin/users` liefert `partner_names: [...]` aggregiert aus drei Quellen: 1) role=partner → eigene Organisation, 2) `partners.linked_user_ids` → explizit verlinkte User, 3) role=user → `progress.data.selected_partner_id/_ids/_name` aus allen partner_selection/partner_multiselection-Steps. Chips pro Partner, truncated + tooltip.
