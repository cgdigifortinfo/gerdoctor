...
## Completed (recent)
- [x] 2026-04-21: **Feature: "Completed Users" Tab im PartnerDashboard** — Users für die der Partner seinen Milestone abgeschlossen hat, wandern automatisch aus "My Users" in die neue Tab "Completed Users". Backend: `/api/partner/submissions` enthält pro Submission einen `partner_work_completed` Boolean (true, wenn alle Milestones in Blocks, in denen der User diesen Partner gewählt hat, auf `completed` stehen). Frontend: Tab-Liste nun 4 statt 3 Tabs (My Users / Completed Users / Other Users / Insights / Profile); `activeSubmissions` + `completedSubmissions` via `useMemo` split. **Regression-Test** `test_partner_completed_users_split.py`: register user → pick Gleichwertigkeits-partner → assert in My Users (pwc=false) → Partner closed milestone → assert in Completed Users (pwc=true) → cleanup. PASS.
