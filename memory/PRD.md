...
## Completed (recent)
- [x] 2026-04-21: **Bugfix: Partner sieht User nicht nach Partner-Wahl** — Root cause: zwei duplizierte "IQB"-Partner-Orgs in DB (Umlaut "IQB Prüfungszentrum" vs ohne "IQB Pruefungszentrum"). Step 12 filter_tag=`Gleichwertigkeitsprüfung` (Umlaut) matcht nur die Umlaut-Variante → User picks und submissions gingen nur dorthin. Login als Partner-User der No-Umlaut-Variante ergab leere "My Users"-Liste. **Fix**: neue Migration `merge_duplicate_iqb_partners.py` merged die No-Umlaut-Variante in die Umlaut-Variante (partner_submissions, user_progress.selected_partner_id/ids, users.partner_id, linked_user_ids union → dann delete victim). Idempotent. **Neuer Regression-Test** `test_partner_selection_visibility.py`: register → durch 12 Steps bis Gleichwertigkeit → Partner wählen → submit → Partner-User login → /partner/submissions enthält User ✓ → cleanup. PASS.
