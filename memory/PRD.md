...
## Completed (recent)
- [x] 2026-04-21: **Bugfix: Milestone blockiert wenn Upload-Pfad gewählt aber keine Datei hochgeladen** — Milestone `auto_complete` Condition von `status_is completed` auf `has_upload(documents)` (mit leerem `value` = "irgendeine Datei") umgestellt. Zusätzlich `block`-Condition mit Compound-`all_of`: {decision=upload} AND {missing_upload documents}. `_evaluate_condition` (Backend + Frontend) unterstützt jetzt `all_of`/`any_of` für UND/ODER-Logik sowie leere `expected`-Werte bei `has_upload`/`missing_upload`. Migration `migrate_milestone_conditions.py` aktualisiert 5 bestehende Milestones (#5, #9, #13, #17, #24) und rollt 2 fälschlich auto-abgeschlossene User-Progress-Rows zurück. E2E Curl-Test bestätigt: ohne Datei → pending + blockiert; mit Datei → completed + unblocked. test_survey_v2.py 12/12 PASS (inkl. neue Block-Assertion). Flowbuilder E2E 15/15 PASS.
