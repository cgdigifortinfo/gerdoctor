...
async def stripe_request(method: str, path: str, *, data: dict | None = None, account_id: str | None = None) -> dict:
    cfg = await stripe_config()
...
async def create_connected_account(email: str, country: str = "DE") -> dict:
...
async def connected_account(account_id: str) -> dict:
...
async def list_invoices(account_id: str) -> dict:
    return await stripe_request("GET", "/invoices?limit=100", account_id=account_id)


async def create_customer(email: str, name: str, partner_id: str) -> dict:
    return await stripe_request("POST", "/customers", data={
        "email": email, "name": name, "metadata[partner_id]": partner_id,
    })


async def create_checkout_session(customer_id: str, price_id: str, partner_id: str, success_url: str, cancel_url: str, mode: str = "subscription", automatic_tax: bool = False, promotion_codes: bool = False) -> dict:
    data = {
...
async def checkout_session(session_id: str) -> dict:
    return await stripe_request("GET", f"/checkout/sessions/{session_id}?expand[]=subscription")


async def create_customer_portal(customer_id: str, return_url: str) -> dict:
